...
def lgb_imp(train_model):
    imp = train_model.feature_importance(importance_type='gain')
    feats = train_model.feature_name()
    df_imp = pd.DataFrame({
        'feats': feats,
        'imp': imp
...
def lgb_model(X_train, y_train, X_valid=None, y_valid=None, valid_model_path='./'):
    ...
    if X_valid != None:
        ...
        # 导出特征重要性
        df_importance = lgb_imp(train_model)
